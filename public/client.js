...
var thisWeek =
  Math.floor(
    (Date.parse(date) - Date.parse("2019-08-31T19:50:00+03:00")) / 604800000
  ) + 1;
...
function display_schedule(data) {
  ...
  for (var d = 0; d < data.length; d++) {
    ...
    for (var i = 0, start = 0, end = 0, total = 0; i < data[d].length; i++) {
      if (data[d][i].weeks.search(" " + curWeek + ",") !== -1) {
        ...
        last.getElementsByClassName("number")[total].innerHTML =
          data[d][i].number;
        last.getElementsByClassName("type")[total].innerHTML = data[d][i].type;
        last.getElementsByClassName("subject")[total].innerHTML =
          data[d][i].class;
        last.getElementsByClassName("classrooms")[total].innerHTML =
          data[d][i].cabinet;
        last.getElementsByClassName("teachers-name")[total].innerHTML =
          data[d][i].teacher;
  ...
  for (var juk = 0; juk < 6; juk++) {
    ...
    curWeek === thisWeek && mobile && Date.now() > dateDay.getTime()
...
function display_exams(data) {
  ...
  for (var d = 0; d < data.length; d++) {
    if (p) {
      ///rewrite without double
      main.innerHTML +=
        '<div class="day"><div onClick="showDay(this)" class="date"><div class="week-day"></div></div></div>';
    } else {
      main.innerHTML +=
